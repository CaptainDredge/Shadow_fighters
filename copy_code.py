...
l_soil=LabelEncoder()
l_soil.fit(train2.typesofsoils)
enc=(list(l_soil.classes_))
enc_soil=np.array(ens)
#df_state=pd.DataFrame(ens)

train2.typesofsoils = l_crop.transform(train2.typesofsoils)
...
def predictor():
   ...
   f=str(data.get("P_r"))
   g=str(data.get("pH"))
   h=str(data.get("typesofsoils")
   array=[]
   array.append([getIndex(state_enc,d),getIndex(enc_year,e),getIndex(enc_season,c),getIndex(enc_soil,h),float(b),float(a),float(g),float(6)])
   array=np.array(array)
   df=pd.DataFrame(array,columns=['State_Name','Crop_Year','Season','Typesofsoil',temperature','Rainfall','pH', 'P_r'])
